...
from compiler.core.util.Position import Position
from compiler.core.error.ExpectedCharError import ExpectedCharError
from compiler.core.error.IllegalCharError import IllegalCharError
from compiler.pi import *
...
class Lexer:
    ...
    def make_tokens(self):
        ...
        while self.current_char is not None:
            if self.current_char in ' \t':
                ...
                tokens.append(Token(Token.TT_PLUS, pos_start=self.pos))
                self.advance()
            elif self.current_char == '-':
    ...
    def make_number(self):
        ...
        if dot_count == 0:
            return Token(Token.TT_INT, int(num_str), pos_start, self.pos)
        else:
            return Token(Token.TT_FLOAT, float(num_str), pos_start, self.pos)
